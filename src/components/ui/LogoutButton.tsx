...
const LogoutButton = ({ onClick, variant = "dark", className = "" }: LogoutButtonProps) => {
  ...
  const updateButtonState = useCallback((newState: string) => {
    if (logoutButtonStates[newState]) {
      setState(newState)
      setStyle((prev) => ({
        ...prev,
        ...logoutButtonStates[newState],
  ...
  return (
    <button
      className={`logoutButton logoutButton--${variant} ${classes.join(" ")} ${className}`}
      style={style}
